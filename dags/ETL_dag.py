...
def get_and_save(**kwargs):
    ...
    with open(
        os.path.join(filepath, f'{str(kwargs["ts_nodash"])}-api-call.txt'), "w"
    ) as f:
...
def flatten_and_upsert_function(**kwargs):
    filepath = FSHook(fs_conn_id="fs_app").get_path()
    df = body_to_df(
        os.path.join(filepath, f'{str(kwargs["ts_nodash"])}-api-call.txt'), 2
    )
    ...
    hook.insert_rows(
        ...
        replace=True,
        replace_index=["Line", "Direction", "Date", "Departure_Time", "Stop"],
    )
...
def flatten_insert_save_buses_function(**kwargs):
    ...
    hook.insert_rows(
        ...
        replace=True,
        replace_index=["Recorded_At", "Line", "Direction", "Date", "Departure_Time"],
    )
    ...
    folium_task = PythonOperator(
        ...
        op_kwargs={
            "in_filename": os.path.join(
                FSHook(fs_conn_id="fs_app").get_path(), "latest_bus_data.csv"
            ),
            "out_filename": os.path.join(
                FSHook(fs_conn_id="fs_app").get_path(), "map.html"
            ),
        },
    )
