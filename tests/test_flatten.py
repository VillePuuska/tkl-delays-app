...
def test_body_to_df():
    ...
    with open('tests/test_data/api-response-1.txt', 'r') as f:
    ...
    pd.testing.assert_frame_equal(df, body_to_df(body))

    # Test with a missing field for one item:
    # recordedAtTime missing for first bus
    with open('tests/test_data/api-response-2.txt', 'r') as f:
        contents = json.loads(f.read())
    body = contents['body']
    ...
    with open('tests/test_data/api-response-3.txt', 'r') as f:
        contents = json.loads(f.read())
    body = contents['body']
...
def test_body_to_df_buses():
    ...
    pd.testing.assert_frame_equal(df, body_to_df_buses(body))
